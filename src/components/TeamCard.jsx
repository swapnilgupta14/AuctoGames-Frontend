        ...
        <div className="flex justify-between px-4 py-6 items-center border-2 border-blue-500 w-[90%] rounded-xl shadow-xl">
            <div className="flex flex-col gap-3">
                <div className="flex justify-center items-center  border-red-500 gap-3">
                    <div>
                        <img src={item.image} alt="" className="w-[65px] h-[65px] rounded-full bg-zinc-300"/>
                    </div>

                    <div className="flex flex-col">
                        <p className="font-[700] text-[14px]">{item.name}</p>
                        <p className="font-[400] text-[12px]">{item.mobileNumber}</p>
                    </div>
                </div>
                ...
                <div className="flex  items-center  border-green-500 gap-2">
                    ...
                    <p className="font-[700] text-[14px]">{item?.totalPoints ? item?.totalPoints : "N/A"}</p>
                </div>
            ...
            <div className="flex flex-col justify-center items-center w-[33%] border border-black rounded-xl py-3 gap-2 cursor-pointer"
            onClick={() => navigate(`/yourTeamPlayers/${auctionId}/${userId}`)}
            >
                <p className="text-center font-[700] text-[14px]">Total Players Bought</p>
                <p className="font-[700] text-[14px]">{item.auctionPlayers.length}/{item.totalPlayers ? item.totalPlayers : "_"}</p>
            </div>
        </div>
    )
}
